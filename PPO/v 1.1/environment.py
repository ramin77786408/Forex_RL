...
class RealForex:
    ...
    def __init__(self, symbol="EURUSD", lot=0.01, max_step_per_episode=20, time_frame=mt5.TIMEFRAME_M15, state_dim=22):
        global episode_reward
...
def get_data_bollinger(symbol, time_frame, number_of_candles, max_period=50):
    # establish connection to MetaTrader 5 terminal
    if not mt5.initialize():
        print("initialize() failed, error code =",mt5.last_error())
        quit()
    ...
    for j in [10,20,50]:
        a[f'indicator_bb_{j}'] = BollingerBands(close=df["close"], window=j, window_dev=3)
        df[f'bb_bbm_{j}'] = a[f'indicator_bb_{j}'].bollinger_mavg()
        df[f'bb_bbh_{j}'] = a[f'indicator_bb_{j}'].bollinger_hband()
        df[f'bb_bbl_{j}'] = a[f'indicator_bb_{j}'].bollinger_lband()
        df[f'bb_bbhi_{j}'] = a[f'indicator_bb_{j}'].bollinger_hband_indicator()    # Add Bollinger Band high indicator
        df[f'bb_bbli_{j}'] = a[f'indicator_bb_{j}'].bollinger_lband_indicator()    # Add Bollinger Band low indicator
